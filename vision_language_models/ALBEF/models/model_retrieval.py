...
class ALBEF(nn.Module):
    def __init__(self, config):
        ...
        if self.distill:
            ...
            self.visual_encoder_m = VisionTransformer(
                img_size=config['image_res'], patch_size=16, embed_dim=768, depth=12, num_heads=12,
                mlp_ratio=4, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), checkpoint=False)
    ...
    def from_cktp(cls, config, ckpt_path=None):
        ...
        model.image_queue = model.image_queue[:, :model.queue_size]
        model.text_queue = model.text_queue[:, :model.queue_size]
        model.idx_queue = model.idx_queue[:, :model.queue_size]
    ...
    def forward(self, image, text, alpha, idx):
        ...
        vl_output_pos = vl_output[:bs]
        pos_matching_score = torch.softmax(vl_output_pos, dim=-1)[:, 1]

        return loss_ita, loss_itm, pos_matching_score
