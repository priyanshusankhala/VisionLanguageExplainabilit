...
from dataset.PromptDataset import PromptDataModule
...
class Trainer:
    ...
    def configure_callbacks(self):
        ...
        callback = [
            ...
            ModelCheckpoint(save_last=True,
                            save_top_k=1,
                            monitor="val_matching_acc",
                            mode='max',
                            verbose=True,
                            save_weights_only=False,
                            filename=self.config.general.experiment_name + "-{epoch}-{val_matching_acc:.2f}",
        ...
        # callback.append(ValidationRetrievalCallback(k_test=100, text_bs=64, verbose=True))
    ...
    def configure_datamodule(self):
        ...
        if self.config.data.dataset == 'coco17':
            def collate_fn(batch):
                images = torch.stack([i[0] for i in batch])
                captions = [i[1] for i in batch]
                token_captions = self.module.model.tokenizer(
                    clean_caption(captions, max_words=self.config.data.max_words),
                    padding='longest',
            ...
            self.datamodule = COCO17DataModule(expand_labels=True,
        ...
        elif self.config.data.dataset == 'coco14+opt':
            def collate_fn(batch):
                images = torch.stack([i['image'] for i in batch])
                captions = [i['label'] for i in batch]
                token_captions = self.module.model.tokenizer(
                    clean_caption(captions, max_words=self.config.data.max_words),
                    padding='longest',
                    max_length=self.config.data.max_words, return_tensors="pt")
                ids = torch.tensor([i['idx'] for i in batch])
                return images, token_captions, ids

            self.datamodule = PromptDataModule(
                train_transform=train_transform(self.config.model.image_res),
                test_transform=test_transform(self.config.model.image_res),
                collate_fn=collate_fn,
                batch_size=self.config.data.batch_size,
    ...
    def fit(self):
        self.prepare()
        self.trainer.fit(datamodule=self.datamodule, model=self.module, ckpt_path=self.config.trainer.ckpt_path)
        self.finish()
    ...
    def validate(self):
        ...
        self.trainer.validate(datamodule=self.datamodule, model=self.module, ckpt_path=self.config.trainer.ckpt_path)
        self.finish()
...
def cmd_line_interface():
    ...
    parser.add_argument('--validate', help="", action='store_true')
    ...
    if config_file.is_file():
        ...
        if args.validate:
            trainer = Trainer(config=config, debug=False)
            trainer.validate()
        else:
