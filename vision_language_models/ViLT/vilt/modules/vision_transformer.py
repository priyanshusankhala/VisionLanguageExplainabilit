...
class Attention(nn.Module):
    ...
    def forward(self, x, mask=None):
        B, N, C = x.shape                                           # 1, 185, 768
        qkv = (
            self.qkv(x)
            .reshape(B, N, 3, self.num_heads, C // self.num_heads)  # 1, 185, 3, 12, 64
            .permute(2, 0, 3, 1, 4)                                 # 3, 1, 12, 185, 64
        )
        ...
        attn = self.attn_drop(attn)  # 1, 12, 185, 185

        if self.save_attention:
            self.save_attention_map(attn)
            attn.register_hook(self.save_attn_gradients)

        x = (attn @ v).transpose(1, 2).reshape(B, N, C)  # 1, 185, 768
        x = self.proj(x)
        x = self.proj_drop(x)
        return x, attn
